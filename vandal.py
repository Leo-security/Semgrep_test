# from ./protype.py import timeoutFn
import csv
import os
import re
import time
...
def get_version(bin_runtime_file):
    ...
    hash_01 = l[-2][:2]
    # sol_path = "./mainnet/" + hash_01.lower() + "/"+ bin_runtime_file.split("/")[-2]
...
def run_cmd(cmd_string, timeout=120):
    print(cmd_string)
    file_bin_path = cmd_string.split(" ")[1]
    ...
    write_csv("../../data/csv_file/vandal_normal.csv", file_bin_path, file_version, file_size, decompile_time, code,
              msg)
...
def decompile(duplicate_path, res_csv_path):
    ...
    with open(duplicate_path, 'r', encoding="utf-8") as csvfile:
        ...
        for row in reader:
            ...
            save_file_name = l[-1].split(".")[0] + "_vandal.txt"  # 保存文件
            vandal_cmd = "../../decompiler/vandal/bin/decompile " + bin_runtime_file + " > " + save_path + save_file_name
            cmd_res.append(vandal_cmd)
...
# get_version("./Decompile/bin-runtime-bytecode_new/05070424a5236b4ef143a6cd7eb82594f9046088_DEEPSHU.sol/Address.bin-runtime")
# decompile("./Decompile/buggy/buggy_bin.csv","./Decompile/buggy/vandal_res.csv")
decompile("../../data/bytecode/normal_bin-runtime.csv", "../../data/csv_file/vandal_normal.csv")

# bin/decompile ../../data/bytecode/normal/0000000000a84fe7f5d858c8a22121c975ff0b42_Poster.sol/Poster.bin-runtime
