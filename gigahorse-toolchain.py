...
def run_cmd(cmd_string, file_path, timeout=120):
    ...
    write_csv("../../data/csv_file/gigahorse_normal.csv", file_path, file_version, file_size, decompile_time, code, msg)
...
def decompile(duplicate_path, res_csv_path):
    ...
    with open(duplicate_path, 'r', encoding="utf-8") as csvfile:
        ...
        for row in reader:
            ...
            save_file_name = l[-1].split(".")[0] + "_gigahorse.json"  # 保存文件
            output_file = save_path + save_file_name
            gigahorse_cmd = f"python {GIGA} {' '.join(ARGS)} -r {output_file} {bin_runtime_file}"
            run_cmd(gigahorse_cmd, bin_runtime_file)


decompile("../../data/bytecode/normal_hex.csv", "../../data/csv_file/gigahorse_normal.csv")
